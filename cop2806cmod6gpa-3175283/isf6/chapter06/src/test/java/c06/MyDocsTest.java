...
import static c06.DocumentType.SPREADSHEET;
...
public class MyDocsTest extends AbstractTestNGSpringContextTests {
  ...
  @Test
  void testFindSpreadSheet() {
    var documents = getEngine().findByType(SPREADSHEET);
    assertNotNull(documents);
    assertEquals(documents.size(), 1);
    assertEquals(SPREADSHEET.name,
            documents.get(0).getType().name);
    assertEquals(SPREADSHEET.desc,
            documents.get(0).getType().desc);
    assertEquals(SPREADSHEET.extension,
            documents.get(0).getType().extension);
  }
